...
#     __settings: settings = settings()
#
#     def __new__(cls): # если new добавим то settings1 и settings2 будут однаковые
#         if not hasattr(cls, 'instance'):
#             cls.instance = super(settings_manager, cls).__new__(cls)
#         return cls.instance
#     # Метод __new__: Этот метод реализует паттерн Singleton,
#     # гарантируя, что существует только один экземпляр класса settings_manager.
#     # При попытке создания нового объекта этого класса будет возвращен уже существующий объект.
#
